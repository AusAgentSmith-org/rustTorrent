...
impl<H: PeerConnectionHandler> PeerConnection<H> {
    ...
    pub async fn manage_peer_incoming(
        ...
        let handshake_supports_dht = incoming.handshake.supports_dht();
    ...
    pub async fn manage_peer_outgoing(
        ...
        async move {
            ...
            let handshake_supports_dht = h.supports_dht();
            trace!(
                peer_id=?h.peer_id,
    ...
    async fn manage_peer(&self, args: ManagePeerArgs) -> Result<()> {
        ...
        if handshake_supports_dht && let Some(port) = self.handler.dht_port() {
            let len = Message::Port(port).serialize(&mut *write_buf, &Default::default)?;
            with_timeout(
                "writing DHT port",
                rwtimeout,
        ...
        let writer = async move {
            ...
            let len = Message::Choke.serialize(&mut *write_buf, &Default::default)?;
            with_timeout(
                "writing",
                rwtimeout,
                write.write_all(&write_buf[..len]).map_err(Error::Write),
            ...
            .await?;
            trace!("sent choke while waiting for an upload slot");

            let _upload_slot = match self.handler.upload_slot_semaphore() {
                Some(semaphore) => Some(
                    semaphore
                        .acquire_owned()
                        .await
                        .map_err(|_| Error::TorrentIsNotLive)?,
                ),
                None => None,
            };
            ...
            .await?;
            trace!("sent unchoke after acquiring an upload slot");
