...
async fn _test_e2e_download(mode: ListenerMode, drop_checks: &DropChecks) {
    ...
    for _ in 0..client_iters {
                ...
                let b = handle
                    .with_state(|s| match s {
                        ...
                        crate::ManagedTorrentState::Paused(p) => {
                            let needed = p.chunk_tracker.get_hns().needed_bytes;
                            if needed == 0 {
                                Ok(true)
                            } else {
                                info!(needed_bytes = needed, "still verifying pieces");
                                Ok(false)
                            }
                        }
                        _ => bail!("bugged state"),
        ...
        .await
        .context("timed out waiting for piece verification to complete (needed_bytes != 0)")
        .unwrap();
